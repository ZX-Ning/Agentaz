...
- `@gotgenes/pi-permission-system`: tool-call permission enforcement and browser
  approval prompts.
...
For offline or pre-bundled deployments, set `AGENTAZ_PI_NODE_MODULES_DIR` to a
`node_modules` directory that already contains these packages. Agentaz then
writes local package roots when present:

```txt
$AGENTAZ_PI_NODE_MODULES_DIR/@juicesharp/rpiv-todo
$AGENTAZ_PI_NODE_MODULES_DIR/@gotgenes/pi-permission-system
```

If either package is missing under that directory, Agentaz warns and falls back
to the corresponding `npm:` source.

The implementation lives in:

```txt
packages/api/src/pi/required-packages.ts
packages/api/src/pi/session-workspace.ts
```

`PiSessionWorkspace` calls `ensureRequiredPiPackages(agentDir)` before
`createAgentSessionServices()`. That writes or updates:

```txt
<agentDir>/settings.json
```

with either `npm:` sources or local package paths selected by
`AGENTAZ_PI_NODE_MODULES_DIR`:
...
{
    "packages": [
        "/path/to/node_modules/@juicesharp/rpiv-todo",
        "/path/to/node_modules/@gotgenes/pi-permission-system"
    ]
}
```

When Pi services are created, the Pi SDK reads that settings file and loads
local package paths directly. For selected `npm:` sources, it installs missing
packages into:
...
and loads extension entrypoints from the packages' Pi manifests, for example:
...
The first run with a new Pi agent directory may need network access because Pi
installs missing packages during resource loading. Once installed, packages live
under the agent directory's `npm/node_modules`.

## Permission Config

The permission extension package and the permission policy config are separate
concerns.
...
and read by `@gotgenes/pi-permission-system` after the extension is loaded. If
the extension package is not loaded, the config file has no effect.
...
- `@gotgenes/pi-permission-system` was listed as an app dependency, but Agentaz
  did not explicitly load it.
- Agentaz then briefly seeded project-local `<cwd>/.pi/settings.json` for
  required packages before switching to the Pi CLI-style agent-dir default.
...
The old `additionalExtensionPaths` logic and duplicate-checking helpers have
been removed. Agentaz now seeds required packages in `<agentDir>/settings.json`.
...
- `@gotgenes/pi-permission-system/package.json` is not exported by its package
  `exports` map, so package root resolution must derive from an exported entry
  or use a copied filesystem path.

This would avoid first-run Pi package installation, but Agentaz would then own
bundling and copying the extension dependency closure correctly. Until that is
needed, Pi-managed packages are simpler and closer to the SDK's native extension
model.
