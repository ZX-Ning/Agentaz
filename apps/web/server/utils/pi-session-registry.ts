...
export class PiSessionRegistry {
  ...
  async attachClient(peer: WsPeer) {
    ...
    const active = this.sessionForClient(
      clientId,
  ...
  private async createSession(clientId: string) {
    ...
    const controller = await PiSessionController.create({
      ...
      onSessionMetadataChanged: () =>
        this.refreshPersistedSessionCache().then(() =>
          this.broadcastSnapshots(),
        ),
    });
  ...
  private async closeSession(sessionId: string, abortCurrent = false) {
    ...
    for (const [activeClientId, activeSessionId] of this
  ...
  private async releaseOneAvailableSessionIfAtCapacity() {
    ...
    if (this.lastActiveSessionId)
      activeSessionIds.add(this.lastActiveSessionId);
  ...
  private sessionForClient(
    ...
    const resolvedSessionId =
      ...
      this.lastActiveSessionId;
    const controller = resolvedSessionId
  ...
  private sendHello(clientId: string) {
    ...
    const active = activeSessionId
      ? this.sessions.get(activeSessionId)
      : undefined;
  ...
  private loadedSessionsFor(clientId: string) {
    return [...this.sessions.values()].map((controller) =>
      controller.toLoadedSession(clientId),
    );
  }
...
export function setPiSessionRegistryConfig(
  options: Omit<RegistryOptions, "maxLoadedSessions"> & {
    maxLoadedSessions?: number;
  },
) {
  const normalized = {
  ...
  if (
    ...
    throw new Error(
      "PiSessionRegistry configuration cannot be changed after it has been set.",
    );
  }
}
