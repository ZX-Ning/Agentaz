...
import {
    ...
    requestClientId,
    requireRouteParam,
    withRequestSessionControl as runWithRequestSessionControl,
} from "../http/agent.ts";
...
type AgentRuntimeProvider = () => AgentRuntime;
...
/** Creates agent routes against an injectable runtime boundary for integration tests. */
export function createAgentRoutes(
    getRuntime: AgentRuntimeProvider = getAgentRuntime,
) {
    const agentRoutes = new Hono();
    const withRequestSessionControl = <T>(
        c: Context,
        sessionId: string,
        run: (
            lease: ReturnType<typeof acquireRequestSessionControl>,
        ) => T | Promise<T>,
    ) => runWithRequestSessionControl(c, sessionId, run, getRuntime());
    ...
    agentRoutes.get(
        ...
        (c) =>
            c.json(
                getConfiguredAgentRegistry(getRuntime()).getDefaultModelState(),
            ),
    );
    ...
    agentRoutes.get("/agent/permissions/config", async (c) => {
        return c.json(
            await getRuntime().workspace.getProjectPermissionConfig(),
        );
    });
    ...
    agentRoutes.put("/agent/permissions/config", async (c) => {
        ...
        return c.json(
            await getRuntime().workspace.setProjectPermissionConfig(
                body.config,
            ),
        );
    });
    ...
    agentRoutes.post("/agent/permissions/config/reset", async (c) => {
        return c.json(
            await getRuntime().workspace.resetProjectPermissionConfig(),
        );
    });
    ...
    agentRoutes.get(
        ...
        (c) =>
            c.json<SessionEntriesResponse>(
                getConfiguredAgentRegistry(getRuntime()).getSessionEntries(
    ...
    agentRoutes.get("/agent/sessions/:sessionId/history", (c) =>
        c.json(
            getConfiguredAgentRegistry(getRuntime()).getSessionHistory(
                requireRouteParam(c, "sessionId"),
            ),
        ));
    ...
    agentRoutes.get("/agent/sessions/:sessionId/models", (c) =>
        c.json(
            getConfiguredAgentRegistry(getRuntime()).getSessionModelState(
                requireRouteParam(c, "sessionId"),
            ),
        ));
    ...
    agentRoutes.post("/agent/sessions/:sessionId/messages", async (c) => {
        ...
        const lease = acquireRequestSessionControl(c, sessionId, getRuntime());
        try {
