...
import { isDraftSessionId, sessionUrl } from "../utils/app.util.ts";
import { findLoadedSessionByFile } from "../utils/agentaz-session-list.ts";
...
import type { AgentazSessions } from "./agentaz-sessions.ts";
...
export function createAgentazActions(
    ...
    function loadDummySession() {
        ...
        ctx.loadedSessions.value = [
            ...ctx.loadedSessions.value.filter((s) =>
        ...
        for (let i = 0; i < 300; i++) {
            ...
            const text = isUser
                ? `This is user message #${
                    i / 2 + 1
                }. How can I help you test the rendering performance of this chat interface? Let me add some more text to make it realistic. `
                    .repeat(
                        3,
                    )
                : `This is assistant response #${
                    Math.floor(i / 2) + 1
                    ...
                    ...(isUser ? [] : [
                        {
                            ...
                            text: `Thinking process for message ${i}... `
                                .repeat(
                                    3,
                                ),
                            collapsed: true,
    ...
    async function clearActiveQueue() {
        if (
            ...
            await agentFetch(
                sessionUrl(ctx.activeSessionId.value, "/queue/clear"),
                {
                    method: "POST",
                },
            );
    ...
    async function compactActiveContext() {
        ...
        toast.add({
            ...
            description:
                `Compacted from ${result.tokensBefore.toLocaleString()} context tokens.`,
            color: "success",
        });
    }
