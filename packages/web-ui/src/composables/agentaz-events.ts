import type { ServerEvent, ServerHello } from "@agentaz/protocol";
import { apiBase, isDraftSessionId } from "../utils/app.util.ts";
import {
    ...
    function handleEvent(event: ServerEvent) {
        ...
        if (event.type === "state_snapshot") {
            ...
            if (
                ...
                void syncBrowserRouteToSession(
                    ctx.activeSessionId.value,
                    "replace",
                );
            }
        ...
        if (event.type === "turn_completed") {
            ...
            void (async () => {
                const knownRevision =
                    ctx.transcriptRevisionBySessionId.value[event.sessionId] ??
                        -1;
        ...
        if (event.type === "turn_failed") {
            ...
            if (event.transcriptRevision !== undefined) {
                ctx.transcriptRevisionBySessionId.value[event.sessionId] = Math
                    .max(
                        ctx.transcriptRevisionBySessionId.value[
                            event.sessionId
                        ] ?? -1,
        ...
        if (event.type === "status") {
            if (event.sessionId) {
                ...
                const shouldRefreshModelState = !event.isStreaming &&
                    (mState.pendingModelChange || mState.pendingThinkingChange);
                const isWorking = event.isStreaming ||
                ...
                if (shouldRefreshModelState) {
                    void refreshModelState(event.sessionId);
                }
            }
            return;
        }
        ...
        return new Promise<ServerHello>((resolve, reject) => {
            ...
            es.addEventListener("message", (message) => {
                try {
                    ...
                    ctx.lastError.value = error instanceof Error
                        ? error.message
                        : String(error);
                    reject(error);
                }
            ...
            es.addEventListener("error", () => {
                ...
                if (
                    !ctx.isUnmounting.value &&
                    !ctx.hasShownDisconnectToast.value
                ) {
