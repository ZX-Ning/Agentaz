...
import { buildUnifiedSessions } from "../utils/agentaz-session-list.ts";
...
export function useAgentazState() {
    ...
    const activeSessionTitle = computed(() => {
        ...
        if (loaded) {
            return sessionTitle(loaded);
        }
        const persisted = persistedSessions.value.find(
            (session) => session.sessionId === sessionId,
        );
        if (persisted) {
            return sessionTitle(persisted);
        }
    ...
    const visibleThinkingOptions = computed(() => {
        const levels = availableThinkingLevels.value.length > 0
