...
import type { ApiFetchOptions } from "./app-fetch.ts";
...
export function createAgentazSessions(
    ...
    async function refreshHistory(sessionId: string, force = false) {
        ...
        if (!force && ctx.messagesBySessionId.value[sessionId]?.length) {
            return;
        }
        ...
        if (history.revision < knownRevision) {
            return;
        }
    ...
    async function postSessionOperation(
        ...
        applyState(state);
        const detailSessionId = ctx.activeSessionId.value ??
    ...
    async function materializeDraftSession(draftSessionId: string) {
        ...
        ctx.modelStateBySessionId.value[sessionId] =
            ctx.modelStateBySessionId.value[draftSessionId] ??
                defaultModelState();
    ...
    async function openPersistedSession(
        ...
        if (!isCurrentSessionSwitchIntent(switchNonce)) {
            return;
        }
        const sessionId = state.sessionId ?? state.activeSessionId;
        rememberSessionFocusIntent(switchNonce, sessionId);
    ...
    async function submitComposer() {
        ...
        if (!isDraftSessionId(sessionId) && ctx.isStreaming.value) {
            await agentFetch(sessionUrl(sessionId, "/abort"), {
                method: "POST",
            });
            return;
        }
        if (ctx.isAwaitingActivePromptResponse.value) {
            return;
        }
        await sendPrompt();
    }
