...
export function createAgentazMutations(ctx: AgentazContext) {
    ...
    function upsertExtensionWidget(
        ...
        if (
            !ctx.loadedSessions.value.some((item) =>
        ...
        const nextWidgets = patch.lines === undefined
     ...
     */
    function moveLocalPromptWorking(
        ...
        const wasWorking =
            ctx.locallyPendingPromptBySessionId.value[fromSessionId];
        if (!wasWorking) {
            return;
        }
